...
実行: python scripts/vegimport.py [--root ../vegitage-data/web/italian]
"""
...
def run(db: Session, root: Path, limit: int | None) -> tuple[int, int]:
    ...
    for path in overviews[:limit]:
        ...
        if existing is not None:
            ...
            if has_article is not None and has_article.current_revision_id:
                skipped += 1
                continue
            variety = existing
        else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--root",
        type=Path,
        default=Path(__file__).parents[2] / "vegitage-data" / "web" / "italian",
    )
    ...
    with maker() as db:
        imported, skipped = run(db, args.root, args.limit)
